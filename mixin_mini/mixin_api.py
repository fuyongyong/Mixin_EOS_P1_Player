...
class MIXIN_API:
    ...
    def genEncrypedPin(self):
        ...
        print(len(paddedContent.encode('utf-8')))
    ...
    def genEncrypedPin(self):
        if self.keyForAES == "":
            ...
            #        print("lenth of decoded pin_token is:" + str(len(decoded_result)))
            # [BEG:UPDATE]
            cipher = PKCS1_OAEP.new(key=privKeyObj, hashAlgo=Crypto.Hash.SHA256, label=self.sessionid.encode("latin-1"))
            # [END:UPDATE]
        ...
        #  print(len(paddedContent.encode('utf-8')))
        # [END:UPDATE]
        encrypted_result = cipher.encrypt(paddedContent.encode('latin-1'))
